...
public class MapView extends FrameLayout {
  ...
  private class MapZoomControllerListener implements ZoomButtonsController.OnZoomListener {
    ...
    public void onZoom(boolean zoomIn) {
      ...
      PointF focalPoint = mapGestureDetector.getFocalPoint();
      if (focalPoint != null) {
        transform.zoom(zoomIn, focalPoint.x, focalPoint.y);
      } else {
        transform.zoom(zoomIn, getMeasuredWidth() / 2, getMeasuredHeight() / 2);
      }
    }
  }
  ...
  private class CameraZoomInvalidator implements TrackingSettings.CameraZoomInvalidator {
    ...
    public void zoomTo(double zoomLevel) {
      ...
      if (currentZoomLevel < zoomLevel) {
        PointF focalPoint = mapGestureDetector.getFocalPoint();
        if (focalPoint != null) {
          transform.setZoom(zoomLevel, focalPoint.x, focalPoint.y);
        } else {
          transform.setZoom(zoomLevel, getMeasuredWidth() / 2, getMeasuredHeight() / 2);
        }
      }
    }
  }
